...
if($_POST)
{
  if(isset($_POST["status"]) || isset($_POST["STATUS"]))
  {
    ...
    if (strpos($designid, 'MVNT') !== false) {
        $buymore = "http://jrntr.stores.zone";
    }
    elseif (strpos($designid, 'MVMH') !== false) {
        $buymore = "http://maheshbabu.stores.zone";
    }
    elseif (strpos($designid, 'MVDR') !== false) {
        $buymore = "http://darshan.stores.zone";
    }
    else
    {
        $buymore = "http://ecomindia.org";
    }
  }
  else
  {
    ...
    try
    {
      ...
      $statement = $db->prepare("INSERT INTO txn VALUES (:txnid, '', DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i:%s'), '', '', :customer_id, :design_id, :product_id, :item_qty, :item_rate, :item_discount, :taxable_value, :cgst_rate, :cgst_amount, :sgst_rate, :sgst_amount, :igst_rate, :igst_amount, :total_amount, :payment_mode, :payment_gateway)");
      $statement->execute($params);
      ...
      $statement2 = $db->prepare("INSERT INTO cust VALUES (:txnid, :customer_email, :customer_id, :customer_name, :customer_address, :customer_city, :customer_state, :customer_pincode)");
    ...
    if($gateway == "paytm")
    {
      ...
      $ORDER_ID = $phone.'_'.$design_id.'_'.rand(10000,99999999);
      $CUST_ID = $txnid;
              ...
              <?php if($_POST) : ?>
                <?php if($isValidChecksum=="TRUE") : ?>
                  <?php if($status=="success" || $status=="TXN_SUCCESS") : ?>       
                    ...
                    <p style="font-size:15px;font-weight: bold;color:#5C5C5C;">Thank you, <?php echo $firstname ?>. You will receive the T-shirt in 2-3 days.</p>
                    <br>
